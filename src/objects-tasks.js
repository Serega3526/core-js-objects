...
function makeWord(lettersObject) {
  const arr = [];
  for (let i = 0; i < Object.values(lettersObject).length; i += 1) {
    for (let j = 0; j < Object.values(lettersObject)[i].length; j += 1) {
      arr[Object.values(lettersObject)[i][j]] = Object.keys(lettersObject)[i];
    }
  }
  return arr.join('');
}
...
function sortCitiesArray(arr) {
  arr.sort((x, y) => x.country.localeCompare(y.country));
  arr.sort((x, y) => {
